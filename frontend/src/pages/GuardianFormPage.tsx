...
export default function GuardianFormPage() {
  ...
  const field = (label: string, key: keyof typeof form, type = 'text', required = false) => (
    <div>
      <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">{label}{required && <span className="text-red-400"> *</span>}</label>
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-6">{isEdit ? '保護者編集' : '保護者新規登録'}</h1>
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">住所</label>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={() => navigate('/guardians')} className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-6 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-600">キャンセル</button>
        </div>
      </form>
    </div>
  )
