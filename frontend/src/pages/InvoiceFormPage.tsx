...
export default function InvoiceFormPage() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-xl font-bold text-gray-100 mb-6">請求書作成</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-200">基本情報</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-200 mb-1">生徒 <span className="text-red-400">*</span></label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-200 mb-1">請求月 <span className="text-red-400">*</span></label>
              <input type="month" value={form.billingMonth} onChange={(e) => setForm({ ...form, billingMonth: e.target.value })} required
                className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm bg-gray-700 text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-200 mb-1">支払期限 <span className="text-red-400">*</span></label>
              <input type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} required
                className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm bg-gray-700 text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-200 mb-1">備考</label>
        ...
        <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-200">請求明細</h2>
          ...
          {items.map((item, i) => (
            <div key={i} className="flex gap-3 items-center">
              <input value={item.description} onChange={(e) => updateItem(i, 'description', e.target.value)}
                ...
                className="flex-1 border border-gray-600 rounded-lg px-3 py-2 text-sm bg-gray-700 text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
              <input type="number" value={item.amount} onChange={(e) => updateItem(i, 'amount', e.target.value)}
                placeholder="金額" min="0"
                className="w-32 border border-gray-600 rounded-lg px-3 py-2 text-sm bg-gray-700 text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
              {items.length > 1 && (
          ...
          <div className="flex justify-end pt-2 border-t border-gray-700">
            <span className="text-sm font-bold text-gray-100">合計: ¥{totalAmount.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => navigate('/invoices')} className="bg-gray-700 text-gray-200 px-6 py-2 rounded-lg text-sm font-medium hover:bg-gray-600">キャンセル</button>
        </div>
      </form>
    </div>
  )
