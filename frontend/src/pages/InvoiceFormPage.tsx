...
export default function InvoiceFormPage() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-xl font-bold text-gray-800 mb-6">請求書作成</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-700">基本情報</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">生徒 <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">請求月 <span className="text-red-500">*</span></label>
              <input type="month" value={form.billingMonth} onChange={(e) => setForm({ ...form, billingMonth: e.target.value })} required
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">支払期限 <span className="text-red-500">*</span></label>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-3">
          ...
          <div className="flex justify-end pt-2 border-t border-gray-100">
            <span className="text-sm font-bold text-gray-800">合計: ¥{totalAmount.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => navigate('/invoices')} className="bg-gray-100 text-gray-700 px-6 py-2 rounded-lg text-sm font-medium hover:bg-gray-200">キャンセル</button>
        </div>
      </form>
    </div>
  )
