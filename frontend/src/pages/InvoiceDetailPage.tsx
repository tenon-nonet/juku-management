  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-100">{invoice.studentName} / {invoice.billingMonth}</h1>
          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${INVOICE_STATUS_COLOR[invoice.status]}`}>
            {INVOICE_STATUS_LABEL[invoice.status]}
          </span>
        </div>
        <Link to="/invoices" className="bg-gray-700 text-gray-200 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-600">一覧へ</Link>
      </div>

      <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6 mb-4">
        <h2 className="text-sm font-semibold text-gray-200 mb-3">請求明細</h2>
        <table className="w-full text-sm mb-3">
          ...
          <tfoot>
            <tr className="border-t-2 border-gray-700">
              <td className="py-2 font-bold text-gray-100">合計</td>
              <td className="py-2 text-right font-bold text-gray-100">¥{invoice.totalAmount.toLocaleString()}</td>
            </tr>
          </tfoot>
        </table>
        <div className="flex items-center justify-between text-sm">
          <span className="text-gray-400">支払期限: {invoice.dueDate}</span>
          <div className="flex gap-2">
            {invoice.status !== 'PAID' && (
              <button onClick={() => handleStatusChange('PAID')} className="text-xs bg-green-900/50 text-green-400 px-3 py-1 rounded-full hover:bg-green-900">入金済にする</button>
            )}
            {invoice.status !== 'CANCELLED' && (
              <button onClick={() => handleStatusChange('CANCELLED')} className="text-xs bg-gray-700 text-gray-300 px-3 py-1 rounded-full hover:bg-gray-600">キャンセル</button>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6 mb-4">
        <h2 className="text-sm font-semibold text-gray-200 mb-3">入金履歴</h2>
        <div className="mb-3 text-sm">
          <span className="text-gray-400">入金済: </span>
            ...
            <tbody className="divide-y divide-gray-700">
              {payments.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="py-2 text-gray-400">{p.recordedBy ?? '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6">
        <h2 className="text-sm font-semibold text-gray-200 mb-3">入金登録</h2>
        <form onSubmit={handleAddPayment} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-gray-400 mb-1 block">入金額 *</label>
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1 block">入金日 *</label>
              <input type="date" value={form.paidAt} onChange={(e) => setForm({ ...form, paidAt: e.target.value })} required
                className="w-full border border-gray-600 rounded-lg px-3 py-2 text-sm bg-gray-700 text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
            </div>
          </div>
          <div>
            <label className="text-xs text-gray-400 mb-1 block">支払方法</label>
