  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">生徒管理</h1>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-4 flex gap-3 flex-wrap">
        ...
        <input
          ...
          className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1.5 text-sm w-36 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400"
        />
        <select
          value={statusFilter}
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">学年</th>
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">学校</th>
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">保護者</th>
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">ステータス</th>
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">入塾日</th>
              <th className="px-4 py-3"></th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {students.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">生徒が見つかりません</td></tr>
            )}
            {students.map((s) => (
              <tr key={s.id} className="hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" onClick={() => navigate(`/students/${s.id}`)}>
                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">
                  {s.fullName}
                  {s.fullNameKana && <span className="block text-xs text-gray-500 dark:text-gray-400">{s.fullNameKana}</span>}
                </td>
                <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{s.grade}</td>
                <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{s.schoolName ?? '-'}</td>
                <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{s.guardianName ?? '-'}</td>
                ...
                <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{s.enrolledAt}</td>
                <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                  <Link to={`/students/${s.id}/edit`} className="text-indigo-600 hover:underline mr-3 text-xs">編集</Link>
                  <button onClick={() => handleDelete(s.id, s.fullName)} className="text-red-400 hover:underline text-xs">削除</button>
                </td>
              </tr>
            ))}
          </tbody>
