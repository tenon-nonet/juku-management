  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-800">授業スケジュール</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        <button onClick={() => setWeekOffset(w => w - 1)} className="p-2 bg-white border border-gray-200 rounded-lg hover:bg-gray-50">←</button>
        <span className="text-sm font-medium text-gray-700 w-48 text-center">{weekLabel}</span>
        <button onClick={() => setWeekOffset(w => w + 1)} className="p-2 bg-white border border-gray-200 rounded-lg hover:bg-gray-50">→</button>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-gray-600">コース</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600">担当</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600">教室</th>
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {lessons.map((l) => {
              ...
              return (
                <tr key={l.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/lessons/${l.id}`)}>
                  <td className="px-4 py-3 text-gray-800">
                    {dt.getMonth() + 1}/{dt.getDate()}（{['日','月','火','水','木','金','土'][dt.getDay()]}）
                    <span className="ml-2 text-gray-500">{dt.getHours()}:{String(dt.getMinutes()).padStart(2, '0')}</span>
                  </td>
                  <td className="px-4 py-3 font-medium text-gray-800">{l.courseName}</td>
                  <td className="px-4 py-3 text-gray-600">{l.teacherName ?? '-'}</td>
