...
import { getDashboardSummary, getTodayLessons, getUnpaidInvoices, getPendingAbsences, getTasks } from '../api'
import type { DashboardSummary, Lesson, Invoice, AbsenceRequest, Task } from '../types'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    getTasks('ACTIVE').then((r) => setUrgentTasks(r.data))
  }, [])
  ...
  const cards = summary ? [
    ...
    { label: '進行中タスク', value: urgentTasks.length, unit: '件', color: 'text-purple-600 dark:text-purple-400', link: '/tasks' },
  ] : []
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        {cards.map((c) => (
          <Link key={c.label} to={c.link} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 hover:shadow-md transition-shadow">
            ...
            <p className={`text-3xl font-bold ${c.color}`}>{c.value}<span className="text-sm font-normal text-gray-500 dark:text-gray-400 ml-1">{c.unit}</span></p>
          </Link>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">本日の授業</h2>
            <Link to="/lessons" className="text-xs text-indigo-600 hover:underline">一覧へ</Link>
          </div>
          {todayLessons.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-gray-400">本日の授業はありません</p>
          ) : (
              ...
              {todayLessons.map((lesson) => {
                ...
                return (
                  <Link key={lesson.id} to={`/lessons/${lesson.id}`}
                    className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-sm">
                    <div>
                      <span className="font-medium text-gray-800 dark:text-gray-100">{lesson.courseName}</span>
                      {lesson.teacherName && <span className="text-gray-500 dark:text-gray-400 ml-2">/ {lesson.teacherName}</span>}
                    </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">未入金請求</h2>
            <Link to="/invoices" className="text-xs text-indigo-600 hover:underline">一覧へ</Link>
          </div>
          {unpaidInvoices.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-gray-400">未入金の請求はありません</p>
          ) : (
            <div className="space-y-2">
              {unpaidInvoices.slice(0, 8).map((inv) => (
                <Link key={inv.id} to={`/invoices/${inv.id}`}
                  className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-sm">
                  <div>
                    <span className="font-medium text-gray-800 dark:text-gray-100">{inv.studentName}</span>
                    <span className="text-gray-500 dark:text-gray-400 ml-2">{inv.billingMonth}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${INVOICE_STATUS_COLOR[inv.status]}`}>
                      {INVOICE_STATUS_LABEL[inv.status]}
                    </span>
                    <span className="font-medium text-gray-800 dark:text-gray-100">¥{inv.totalAmount.toLocaleString()}</span>
                  </div>
                </Link>
              ))}
              {unpaidInvoices.length > 8 && (
                <p className="text-xs text-gray-500 dark:text-gray-400 text-center pt-1">他 {unpaidInvoices.length - 8} 件</p>
              )}
            </div>
          )}
        </div>
