...
export default function LessonFormPage() {
  ...
  return (
    <div className="max-w-xl">
      <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-6">{isEdit ? '授業編集' : '授業追加'}</h1>
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">コース <span className="text-red-400">*</span></label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">日時 <span className="text-red-400">*</span></label>
            <input type="datetime-local" value={form.scheduledAt} onChange={(e) => setForm({ ...form, scheduledAt: e.target.value })} required
              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">時間（分）</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">メモ</label>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={() => navigate('/lessons')} className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-6 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-600">キャンセル</button>
        </div>
      </form>
    </div>
  )
