  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-100">保護者管理</h1>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-900 border-b border-gray-700">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-gray-400">電話番号</th>
              <th className="text-left px-4 py-3 font-medium text-gray-400">メール</th>
              <th className="text-left px-4 py-3 font-medium text-gray-400">住所</th>
          ...
          <tbody className="divide-y divide-gray-700">
            ...
            {guardians.map((g) => (
              <tr key={g.id} className="hover:bg-gray-700 cursor-pointer" onClick={() => navigate(`/guardians/${g.id}`)}>
                <td className="px-4 py-3 font-medium text-gray-100">
                  {g.fullName}
                  {g.fullNameKana && <span className="block text-xs text-gray-400">{g.fullNameKana}</span>}
                </td>
                <td className="px-4 py-3 text-gray-400">{g.phone}</td>
                <td className="px-4 py-3 text-gray-400">{g.email ?? '-'}</td>
                <td className="px-4 py-3 text-gray-400 max-w-xs truncate">{g.address ?? '-'}</td>
