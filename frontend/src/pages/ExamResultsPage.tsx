  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-800">成績記録</h1>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-4 space-y-3">
          <h2 className="text-sm font-semibold text-gray-700">新規登録</h2>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs text-gray-600 mb-1 block">生徒 *</label>
            ...
            <div>
              <label className="text-xs text-gray-600 mb-1 block">テスト種別</label>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-gray-600">テスト名</th>
              ...
              <th className="text-right px-4 py-3 font-medium text-gray-600">得点</th>
              <th className="text-right px-4 py-3 font-medium text-gray-600">割合</th>
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {results.map((r) => (
              <tr key={r.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-600">{r.examName}</td>
                ...
                <td className="px-4 py-3 text-right text-gray-600">{(Number(r.score) / Number(r.maxScore) * 100).toFixed(1)}%</td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => handleDelete(r.id)} className="text-red-500 hover:underline text-xs">削除</button>
                </td>
              </tr>
            ))}
          </tbody>
